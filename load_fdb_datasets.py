...
def add_noise(df, noise_type, noise_amount, *, time_index=None, features=None, cat_features=None, label=None):
    ...
    if noise_type == 'time-dependent':
        ...
        if noise_type == 'boundary-consistent':
            ...
            X = target_encoder.fit_transform(df_copy[features], df_copy[label])
            clf = CatBoostClassifier(verbose=False)
            clf.fit(X, df_copy[label])
            _, noisy_labels = BCNoise(clf, noise_level=s).simulate_noise(X, df_copy[label])
        else:        
...
def train_valid_split(df, split=0.7, shuffle=True, sort_key='creation_date'):
    if shuffle:
        ...
        df = df.sort_values(by=sort_key, ignore_index=True)#.copy()? does sort_values return a copy?
...
def prepare_noisy_dataset(key, noise_type, noise_amount, split=0.7, shuffle=True, sort_key='creation_date'):
    """
    ...
    if key == 'cloud':
        try:
            from cloud_dataset import prepare_data_cloud
            df, features, cat_features, label, record_id = prepare_data_cloud()
            train_and_valid = df[df.dataset.isin(['train','valid'])]
        except ModuleNotFoundError as e:
            print('this dataset is unavailable')
            return
    else:
